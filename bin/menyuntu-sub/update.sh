CHOICES=(
	"Menyuntu        Update Menyuntu itself and run any migrations"
...
if [[ "$CHOICE" == "<< Back"* ]] || [[ -z "$CHOICE" ]]; then
	...
	case "$INSTALLER" in
	"menyuntu") INSTALLER_FILE="$MENYUNTU_PATH/bin/menyuntu-sub/migrate.sh" ;;
	"ollama") INSTALLER_FILE="$MENYUNTU_PATH/install/terminal/optional/app-ollama.sh" ;;
	*) INSTALLER_FILE="$MENYUNTU_PATH/install/terminal/app-$INSTALLER.sh" ;;
	esac
