set_font() {
	...
	if ! $(fc-list | grep -i "$font_name" >/dev/null); then
		...
		cd -
		clear
		source $MENYUNTU_PATH/ascii.sh
	fi

	gsettings set org.gnome.desktop.interface monospace-font-name "$font_name 10"
	cp "$MENYUNTU_PATH/configs/alacritty/fonts/$file_name.toml" ~/.config/alacritty/font.toml
...
"Meslo")
	...
	;;
"> Change size")
	source $MENYUNTU_PATH/bin/menyuntu-sub/font-size.sh
	exit
	;;
esac
